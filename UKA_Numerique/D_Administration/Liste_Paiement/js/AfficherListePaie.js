  ...
  function AfficherListePaiement(code_prom,idAnnee_Ac)
{
   ...
   let TabListe = document.getElementById("TabListePaie");
    ...
    td7.textContent = "Reste FA";
    tdA7.textContent = "Enrol Miss";
      ...
      data.forEach(infos =>
        {
          // Création de TR
              ...
              var tdEnrol7 = document.createElement("td");
              ...
              tdEnrolMiss.textContent=document.getElementById("FA").innerText-infos.Montant;
              tdEnrol7.textContent=infos.Enrol_S_1;
              ...
              if(session="Mi-Session" && MiSession.checked)
                {
                  ...
                  if (infos.Montant >= montanpaye && infos.Enrol_S_1 == enrolement)
                    {
                      ...
                      countEnOrdre++; // Incrémenter le compteur "En ordre"
                    } 
                  else
                  {
                    tdObs.textContent="Pas en ordre";
                    countPasEnOrdre++; // Incrémenter le compteur "Pas en ordre"
                  } 
                }
              else if(session="D4" && d4.checked)
                  {
                    ...
                    if (infos.Montant == montanpaye)
                      {
                        tdObs.textContent="En ordre"; countEnOrdre++;
                      } 
                    else
                    {
                      if (infos.Montant == 0)
                      ...
                      tdObs.textContent="Pas en ordre"; countPasEnOrdre++;
                    } 
                  }
          ...
          // Sélectionner les éléments span par leur ID
          var totalEnOrdreSpan = document.getElementById("TotalEnOrdre");
          var totalPasEnOrdreSpan = document.getElementById("TotalPasEnOrdre");
          var totalRien = document.getElementById("TotalRienPayé");

          // Ajouter les valeurs aux spans
          totalEnOrdreSpan.textContent = `Total étudiant en ordre: ${countEnOrdre}`;
          totalPasEnOrdreSpan.textContent = `Total étudiant pas en ordre: ${countPasEnOrdre}`;
          totalRien.textContent = `Total étudiant avec zéro frais payé: ${countRienPayé}`;
