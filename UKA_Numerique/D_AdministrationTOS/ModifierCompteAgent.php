        ...
        if(isset($_POST['ModifierPhoto'])) 
          {
            ...
            { 
              echo "bonjour";
                ...
                $sql = "UPDATE compte_agent SET Photo_profil=:photo, Nom_image=:nom, Type_image=:typeimage
                WHERE Mat_agent =:matricule";
                $stmt = $con->prepare($sql);
                $stmt->bindParam(':photo', file_get_contents($_FILES["Imageprofil"]["tmp_name"]));
                $stmt->bindParam(':nom', $_FILES["Imageprofil"]["name"]);
                $stmt->bindParam(':typeimage', $_FILES["Imageprofil"]["type"]);
                $stmt->bindParam(':matricule',  $_SESSION['agent']['matr']);

                echo "Matricule".$_SESSION['agent']['matr'];
