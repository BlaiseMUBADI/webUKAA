...
// +-------------------------------------------------+
// $Id: oai.class.php,v 1.6.6.1 2012-02-16 10:31:37 jpermanne Exp $
...
class oai_source extends connecteur_out_source {
	...
	function get_config_form() {
		...
		//Include items
		$result .=	'<div class=row><input onchange="document.getElementById(\'include_items\').disabled = !document.getElementById(\'include_items\').checked;" id="include_items" '.($this->include_items ? 'checked' : '').' name="include_items" type="checkbox">'.'<label class="etiquette" for="include_items">'.$this->msg["include_items"].'</label><br />';
		$result .=	'</div>';
	...
	function update_config_from_form() {
		...
		global $repo_name, $admin_email, $included_sets, $repositoryIdentifier, $chunksize, $token_lifeduration, $cache_complete_records, $cache_complete_records_seconds, $link_status_to_deletion, $linked_status_to_deletion, $allow_gzip_compression, $baseURL, $include_items;
