...
// +-------------------------------------------------+
// $Id: export.inc.php,v 1.2.6.2 2012-04-03 09:51:55 arenou Exp $
...
//	}
	...
	$isbd=str_replace("<br />","\n",$titre_de_forme.$display_isbd.$langues);
...
//				$mention_resp_lib .= ", ".$fonction_auteur[$auteur_2["fonction"]];
		...
		if($pmb_show_notice_id){
			...
			$display_isbd .= "<br /><b>".$msg['notice_id_libelle']."&nbsp;</b>".($prefixe[1] ? $prefixe[1] : '').$obj->notice_id."<br />";
		}
		// fin du niveau 1
		if($obj->level == 1) {
			if ($obj->show_explnum) {
