...
// +-------------------------------------------------+
// $Id: func_bcdi.inc.php,v 1.8.6.1 2012-03-08 16:07:45 dgoron Exp $
...
function import_new_notice_suite() {
	...
	for ($i=0; $i<count($info_606_a); $i++) {
		for ($j=0; $j<count($info_606_a[$i]); $j++) {
			...
			//Recherche du terme
			$requete="SELECT categ_id FROM categories WHERE categ_libelle='".addslashes($descripteur)."'";
			$resultat=mysql_query($requete);
			if (@mysql_num_rows($resultat)) {
				$categ_id=mysql_result($resultat,0,0);
				$requete="INSERT INTO notices_categories (notcateg_notice,notcateg_categorie,ordre_categorie) VALUES($notice_id,$categ_id,$ordre_categ)";
				mysql_query($requete);
				$ordre_categ++;
			} else {
	...
	if ($unknown_desc) {
		...
		$requete="insert into notices (index_l, index_matieres) values('".addslashes($mots_cles)."','".addslashes(strip_empty_words($mots_cles))."')";
		mysql_query($requete);
	}
