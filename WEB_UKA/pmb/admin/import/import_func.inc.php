...
// +-------------------------------------------------+
// $Id: import_func.inc.php,v 1.93.2.1 2012-03-28 12:04:47 dgoron Exp $
	...
	if($bibliographic_level != "s"){
		...
		*/
		...
		for ($i=0; $i<count($lang_code); $i++) {
			...
			$rqt_ins = "insert into notices_langues (num_notice, type_langue, code_langue, ordre_langue) VALUES ('$notice_id',0, '".addslashes($lang_code[$i])."','$i') " ;
			@mysql_query($rqt_ins, $dbh); 
		}
		for ($i=0; $i<count($org_lang_code); $i++) {
			$org_lang_code[$i]=trim($org_lang_code[$i]);
			$rqt_ins = "insert into notices_langues (num_notice, type_langue, code_langue, ordre_langue) VALUES ('$notice_id',1, '".addslashes($org_lang_code[$i])."','$i') " ;
			@mysql_query($rqt_ins, $dbh);
		}
