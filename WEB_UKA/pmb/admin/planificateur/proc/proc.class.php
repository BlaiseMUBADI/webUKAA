...
// +-------------------------------------------------+
// $Id: proc.class.php,v 1.1 2011-07-29 12:32:12 dgoron Exp $
			...
			<div class='row'>
				...
				<div class='colonne_suite'>
					<select id='form_procs' class='saisie-60em' name='form_procs' onchange='reload(this);'>
						...
						$requete = "SELECT idproc, name FROM procs";
		...
		if (SESSrights & ADMINISTRATION_AUTH) {
			...
			if ($parameters["type_proc"]) {
				if ($parameters["type_proc"] == 'internal') {
					...
					if (mysql_num_rows($res) == 1) {
						...
						if($this->statut == RUNNING) {
							...
							if (method_exists($this->proxy, "pmbesProcs_executeProc")) {
								$result_proc = $this->proxy->pmbesProcs_executeProc(INTERNAL,$id_proc,$parameters["envt"],100);
					...
					if($this->statut == RUNNING) {
						if (method_exists($this->proxy, "pmbesProcs_executeProc")) {
							$result_proc = $this->proxy->pmbesProcs_executeProc(EXTERNAL,$id_proc,$parameters["envt"],100);
