...
// +-------------------------------------------------+
// $Id: reader.class.php,v 1.1 2011-07-29 12:32:13 dgoron Exp $
...
//											generateMailReadersEndSubscription($ligne["id_empr"],$empr_location_id);	
								...
								if ($object_fpdf) {
									//génération du pdf
									$this->generate_docnum($id_tache,$object_fpdf);
								}
							} else {
...
//						break;
	...
	function traite_commande($cmd,$message) {		
		switch ($cmd) {
			case RESUME:
				$state = $this->send_command($this->id_tache,WAITING);
				break;
			case SUSPEND:
//				$state = SUSPENDED;
				break;
