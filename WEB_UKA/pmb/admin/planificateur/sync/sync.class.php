...
// +-------------------------------------------------+
// $Id: sync.class.php,v 1.1 2011-07-29 12:32:14 dgoron Exp $
...
class sync extends tache {
	...
	function task_execution($id_tache) {
		...
		if (SESSrights & ADMINISTRATION_AUTH) {
			...
			$tparameters = $this->unserialize_task_params($id_tache);
			...
			$id_source = $source_entrepot;
			if ($id_source) {
				$rqt = "select id_connector, name from connectors_sources where source_id=".$id_source;
				$res = mysql_query($rqt);
				...
				for ($i=0; $i<count($param["ITEM"]); $i++) {
					...
					if ($item["PATH"] == $path) {
						$id = $item["ID"];
						if ($id) {
							$this->id_connector = $id;
							$this->id_source = $id_source;
							$this->id_tache = $id_tache;
							$result = array();
							$this->report[] = "<tr ><th>".$this->msg["report_sync"]." : ".$name."</th></tr>";
							if (method_exists($this->proxy, "pmbesSync_doSync")) {
								$result[] = $this->proxy->pmbesSync_doSync($id, $id_source,$id_tache, array(&$this, "listen_commande"), array(&$this, "traite_commande"));
								if ($result) {
							...
							//fin de l'éxécution - mise à jour de la progression 
//							$this->update_progression($id_tache, 100);	
	...
	function traite_commande($cmd,$message) {
		...
		switch ($cmd) {
			case RESUME:
				$this->send_command($this->id_tache,RUNNING);
				break;
			case SUSPEND:
				$this->suspend_sync();
				break;
...
//				break;
	...
	function make_serialized_task_params() {
    	global $base_path, $source_entrepot, $connecteurId, $task_id;
	...
	function suspend_sync() {
		while ($this->statut == SUSPENDED) {
			sleep(20);
			$this->statut = $this->listen_commande($this->id_tache, array(&$this,"traite_commande"));
		}
	}
