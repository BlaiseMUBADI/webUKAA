...
// +-------------------------------------------------+
// $Id: update_notice.inc.php,v 1.68.4.2 2012-03-28 12:05:28 dgoron Exp $
...
if ($acces_m==0) {
	...
	if(($f_tit1)&&trim($f_tit1)&&(!$nberrors)) {
		...
		$ind_wew = $ind_serie." ".$t_notice['tnvol']." ".$t_notice['tit1']." ".$t_notice['tit2']." ".$t_notice['tit3']." ".$t_notice['tit4'] ;
		...
		if ($f_indexation) $ind_libre = ' '.strip_empty_words(str_replace($pmb_keyword_sep," ",$f_indexation)).' ';
		else $ind_libre = '';
