...
// +-------------------------------------------------+
// $Id: pret.inc.php,v 1.84.2.3 2012-01-18 08:57:49 dgoron Exp $
...
if($confirm_pret && $id_empr){
	...
	if(is_array($id_expl)) {
		foreach($id_expl as $id) {
			if($id)$status= $expl->confirm_pret($id_empr, $id);		
		}
	} else {
		if($id_expl)$status = $expl->confirm_pret($id_empr, $id_expl);
	}
	$erreur_affichage = "<hr />
		<div class='row'>
							...
							if (!$confirm) {
								...
								if ($statut -> flag == $confirm) {
									...
									*/
									...
									if ($statut->expl_comment) $erreur_affichage.= "<div class='erreur'>".$statut->expl_comment."</div>";
...
function check_document($id_expl, $id_empr) {
	...
	$query = "select expl_cote, location_libelle, section_libelle, tdoc_libelle, e.expl_cb as cb, e.expl_id as id, e.expl_location, s.pret_flag as pretable, e.expl_notice as notice, e.expl_bulletin as bulletin, e.expl_note as note, expl_comment, s.statut_libelle as statut";
