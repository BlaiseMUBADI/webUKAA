...
// +-------------------------------------------------+
// $Id: consolidation.class.php,v 1.4.4.3 2011-12-30 08:04:07 dbellamy Exp $
...
class consolidation {
	...
	function calculer_since_last($id_vue){
		...
		if ($this->flag || $this->remove_data || $this->date_consolidation=='0000-00-00 00:00:00') {
			$req = "create temporary table logs_filtre_$id_vue select * from statopac where date_log <= now()";
			mysql_query($req,$dbh);
			$this->set_dates_log($id_vue);
		} else {
			$req = "create temporary table logs_filtre_$id_vue select * from statopac where date_log > '".$this->date_consolidation."' ";
			mysql_query($req,$dbh);
			$this->set_dates_log($id_vue,true);
		}
	}
	...
	function set_dates_log($id_vue,$only_last=false) {
		global $dbh;
		if (!$id_vue) return;
		$q = 'select '.(($only_last)?'min(date_log) as min_date,':'').'max(date_log) as max_date from logs_filtre_'.$id_vue;
		$r = mysql_query($q, $dbh);
		if (mysql_num_rows($r)) {
			$row = mysql_fetch_object($r);
			if (!$only_last) $this->date_debut_log = $row->min_date;
