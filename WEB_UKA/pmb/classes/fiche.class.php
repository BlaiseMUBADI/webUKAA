...
// +-------------------------------------------------+
// $Id: fiche.class.php,v 1.9.2.2 2012-02-09 11:30:46 dbellamy Exp $
...
class fiche{
	...
	function show_edit_form(){
		...
		$form_edit_fiche=str_replace('<!-- focus -->',"<script type='text/javascript'>document.getElementById('".$perso_['FIELDS'][0]['NAME']."').focus();</script>",$form_edit_fiche);
	...
	function get_values($id_fiche,$visible=0){
		...
		if ((!$this->p_perso->no_special_fields)&&($id_fiche)) {
			...
			while ($r=mysql_fetch_array($resultat)) {
				...
				} else if (($this->p_perso->t_fields[$r[$this->p_perso->prefix."_custom_champ"]]["TYPE"]) == "url") {
					$url = explode ("|",$r[$this->p_perso->prefix."_custom_".$this->p_perso->t_fields[$r[$this->p_perso->prefix."_custom_champ"]]["DATATYPE"]]);
					//il existe un libellé
					if ($url[1])
						$values[$r[$this->p_perso->prefix."_custom_champ"]][]= "<a href='".$url[0]."' target='_blank'>".$url[1]."</a>";
					else
						$values[$r[$this->p_perso->prefix."_custom_champ"]][]= "<a href='".$url[0]."' target='_blank'>".$url[0]."</a>";
				} else {
	...
	function display_results_tableau($liste_result,$back_url="",$i_search_deb=0){
		...
		foreach($liste_result as $index=>$liste){
			...
			$display .= "<tr style='cursor: pointer' $class onclick=\"document.location='./fichier.php?categ=consult&mode=search&sub=view&perso_word=$perso_word&page=$page&idfiche=$index&i_search=".$i_search_deb++."';\">";
