...
// +-------------------------------------------------+
// $Id: indexation_docnum.class.php,v 1.19.4.1 2011-12-29 11:13:21 mbertin Exp $
		...
		while(($explnum = mysql_fetch_object($result_expl))){
			if($explnum->explnum_data)
				...
				if($path){
					...
					if(file_exists($path)){
						$fp = fopen($path , "r" ) ;
						if((filesize($path) < $this->return_bytes(ini_get('upload_max_filesize')) && (filesize($path) < (($this->return_bytes(ini_get('memory_limit'))*1)-(memory_get_usage(true)*1))))){
