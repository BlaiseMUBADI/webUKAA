...
// +-------------------------------------------------+
// $Id: noeuds.class.php,v 1.22.2.4 2012-04-05 15:30:21 arenou Exp $
...
//			$this->process_categ_path ($row->id_noeud,$path);
	...
	function listUsedInSeeAlso($id_noeud=0) {
		...
		if(!$id_noeud && (is_object($this))) $id_noeud = $this->id_noeud; 	
		$q = "select if(num_noeud_orig!= ".$id_noeud.",num_noeud_orig,num_noeud_dest)as id_noeud from voir_aussi where num_noeud_orig = '".$id_noeud."' ";
		$q.= "or num_noeud_dest = '".$id_noeud."' ";
		$r = mysql_query($q, $dbh);
		return $r;
	}
