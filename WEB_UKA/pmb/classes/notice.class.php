...
// +-------------------------------------------------+
// $Id: notice.class.php,v 1.125.2.4 2012-02-17 13:32:14 arenou Exp $
			...
			if ($champ_trouve) {
				...
				foreach($temp_ext as $k=>$v) {
					...
					for ($i = 0; $i<count($v["TABLE"]); $i++) {
						...
						$query.=$jointure." where ".$temp_not['t'][0][0].".".$tableau['REFERENCEKEY'][0][value]."=".$notice;
						if($table['LANGUAGE']){
							$tab_req_lang[$k].=$jointure." where ".$temp_not['t'][0][0].".".$tableau['REFERENCEKEY'][0][value]."=".$notice;
						}
				...
				if($datatype=='all') {
					...
					foreach ( $tab_code_champ as $val ) {
       					$req_del="delete from notices_mots_global_index where id_notice='".$notice."' and code_champ='".$val['code_champ']."'";
						mysql_query($req_del,$dbh);
						//la table pour les recherche exacte
						$req_del="delete from notices_fields_global_index where id_notice='".$notice."' and code_champ='".$val['code_champ']."'";
						mysql_query($req_del,$dbh);	
					}
					...
					if(count($tab_pp)){
						foreach ( $tab_pp as $id ) {
	       					$req_del="delete from notices_mots_global_index where id_notice='".$notice."' and code_champ='".$id."' ";
							mysql_query($req_del,$dbh);
							//la table pour les recherche exacte
							$req_del="delete from notices_fields_global_index where id_notice='".$notice."' and code_champ='".$id."' ";
							mysql_query($req_del,$dbh);	
						}
					}
				}
				...
				foreach($tab_req as $k=>$v) {	
					...
					foreach ($tab_mots as $nom_champ=>$tab) {
						...
						foreach ( $tab as $mot => $lang ) {
							//la quantité pas besoin...
       						//$tab_insert[]="(".$notice.",".$tab_code_champ[$v["table"]][$nom_champ][0].",".$tab_code_champ[$v["table"]][$nom_champ][1].",'".$mot."','$lang',".$tab_code_champ[$v["table"]][$nom_champ][2].",$pos)";
							$tab_insert[]="(".$notice.",".$tab_code_champ[$k][$nom_champ]['champ'].",".$tab_code_champ[$k][$nom_champ]['ss_champ'].",'".$mot."','$lang',".$tab_code_champ[$k][$nom_champ]['pond'].",$pos)";
							$pos++;
						}
					}
					//la table pour les recherche exacte
					foreach ($tab_fields as $nom_champ=>$tab) {
						foreach($tab as $order => $values){
       						...
       						$tab_field_insert[]="(".$notice.",".$tab_code_champ[$k][$nom_champ]['champ'].",".$tab_code_champ[$k][$nom_champ]['ss_champ'].",".$order.",'".addslashes($values['value'])."','".addslashes($values['lang'])."',".$tab_code_champ[$k][$nom_champ]['pond'].")";
						}
					}
				}
				//Les champs perso
					...
					foreach ( $tab_pp as $code_champ => $table ) {
       					...
       					if(count($ids)){
       						for($i=0 ; $i<count($ids) ; $i++) {
	      						$data=$p_perso->get_fields_recherche_mot($ids[$i]);
		       					$j=0;
	       						foreach ( $data as $code_ss_champ => $value ) {
	       							$tab_mots=array();
	       							$tab_tmp=explode(' ',strip_empty_words($value));
		       						//la table pour les recherche exacte
		       						$tab_field_insert[]="(".$notice.",".$code_champ.",".$code_ss_champ.",".$j.",'".addslashes(trim($value))."','',".$p_perso->get_pond($code_ss_champ).")";
	    	   						$j++;
									foreach($tab_tmp as $mot) {
										if(trim($mot)){
											$tab_mots[$mot]= "";
									...
									foreach ( $tab_mots as $mot => $lang ) {
										$tab_insert[]="(".$notice.",".$code_champ.",".$code_ss_champ.",'".$mot."','$lang',".$p_perso->get_pond($code_ss_champ).",$pos)";
										$pos++;
									}
								}
       						}
				...
				$req_insert="insert into notices_mots_global_index(id_notice,code_champ,code_ss_champ,mot,lang,pond,position) values ".implode(',',$tab_insert);
