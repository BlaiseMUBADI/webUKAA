...
class tache_calendar {
	...
	function calcul_next_exec($num_planificateur) {
		...
		$sql = "SELECT p.id_planificateur, p.num_type_tache, p.libelle_tache, p.perio_heure, p.perio_minute,
			p.perio_jour, p.perio_mois FROM planificateur p 
				Where p.id_planificateur=".$num_planificateur;
		...
		if ($mois_bdd[0] != '*') {
			...
			for ($i=0; $i<sizeof($mois_bdd); $i++) {
				if (($mois_bdd[$i] >= $this->new_date["MOIS"]) && (!$annee_courante)) {
					$annee_courante = true;
					$mois = $mois_bdd[$i];
//					$this->new_date["MOIS"] = $mois_bdd[$i];
				}
			}
			if (!$annee_courante) {
				$this->new_date["ANNEE"]++;
				$mois = $mois_bdd[0];
//				$this->new_date["MOIS"] = $mois_bdd[0];	
			}	
		} 
		else {
	...
	function calcul_minute_exec($minute_bdd,$date_today,$date_du_jour,$jour_semaine_bdd, $mois_bdd) {
		...
		if ($minute_bdd != '*') {
			...
			if (ereg("^([0-9]{1})$", $minute_bdd) || ereg("^([0-9]{2})$", $minute_bdd)) {
...
//						else if ($val > $date_du_jour["minutes"]) {
//							$this->new_date["MINUTE"] = $tab_m[0];
//						} 
						else {
...
//					$this->new_date["HEURE"]++;
	...
	//calcul de l'heure, analyse de la chaîne
		if ($heure_bdd != '*') {
			// heure saisie au format statique, exemple: 3 ou 03 pour 3h
			if (ereg("^([0-9]{1})$", $heure_bdd) || ereg("^([0-9]{2})$", $heure_bdd)) {
