...
// +-------------------------------------------------+
// $Id: z3950_notice.class.php,v 1.121.2.4 2012-03-28 12:06:49 dgoron Exp $
	...
	function insert_in_database () {
		...
		while (list ($key, $val) = each ($f_lang_form)) {
			...
			if ($tmpcode_langue) {
				...
				$rqt = $rqt_ins . " ('$notice_retour',0, '$tmpcode_langue', $tmpordre_langue) " ; 
				$res_ins = mysql_query($rqt, $dbh);
			}
		}
		...
		while (list ($key, $val) = each ($f_langorg_form)) {
			...
			if ($tmpcode_langue) {
				$tmpordre_langue = $val['ordre'];
				$rqt = $rqt_ins . " ('$notice_retour',1, '$tmpcode_langue', $tmpordre_langue) " ; 
				$res_ins = @mysql_query($rqt, $dbh);
			}
		}
...
//			$alink = '<a target="_blank" href="'.htmlspecialchars($doc_num["a"]).'">'.htmlspecialchars($doc_num["a"]).'</a>';
	...
	function from_form () {
		...
		if ($author0_type == "use_existing") {
			...
			global $f_author_name_0, $f_author_rejete_0, $f_author_date_0, $f_author_type_0, $f_author_function_0, $f_author_lieu_0, $f_author_pays_0, $f_author_comment_0, $f_author_ville_0, $f_author_subdivision_0, $f_author_numero_0, $f_author_web_0  ;
			$this->aut_array[] = array(
		...
		for ($i=0; $i< $max_aut1 ; $i++) {
			...
			global $$var_aut_name, $$var_aut_rejete, $$var_aut_date, $$var_aut_type_auteur, $$var_aut_function, $$var_auth_type_use, $$var_aut_lieu, $$var_aut_pays, $$var_aut_comment, $$var_aut_ville, $$var_aut_subdivision, $$var_aut_numero, $$var_aut_web;
	...
	function from_unimarc ($record) {
		...
		*/
		...
		$this->serie = clean_string ($serie_200[0][i]);
		if ($this->serie){
			$this->nbr_in_serie = $serie_200[0][h];
		}else{
