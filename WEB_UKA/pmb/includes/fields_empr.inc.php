...
// +-------------------------------------------------+
// $Id: fields_empr.inc.php,v 1.63.2.1 2012-03-14 10:20:29 dgoron Exp $
...
$aff_filter_list_empr=array("text"=>"aff_filter_text_empr","list"=>"aff_filter_list_empr","query_list"=>"aff_filter_query_list_empr","date_box"=>"aff_filter_date_box_empr","comment"=>"aff_filter_comment_empr","external"=>"aff_filter_external_empr","url"=>"aff_filter_resolve_empr","resolve"=>"aff_filter_resolve_empr");
...
function aff_url_empr($field,&$check_scripts){
	...
	foreach ($values as $avalues) {
		...
		$ret.= $msg['persofield_url_link']."<input id='".$field[NAME]."_link' type='text' class='saisie-30em' name='".$field[NAME]."[link][]' onchange='cp_chklnk($count,this);' value='".htmlentities($avalues[0],ENT_QUOTES,$charset)."'>";
		$ret.=" <input class=\"bouton\" type='button' value='".$msg['persofield_url_check']."' onclick='cp_chklnk($count,this);'>";
		//$ret.="<br />";
	...
	<script type='text/javascript'>
		function cp_chklnk(indice,element){
	...
	if ($options[REPEATABLE][0][value]) {
		...
		$ret.="<script>
			function add_custom_url_(field_id, field_name, field_size) {
				...
				chklnk.setAttribute('onclick','cp_chklnk('+cpt+',this);');
				...
				link.setAttribute('onchange','cp_chklnk('+cpt+',this);');
