...
// +-------------------------------------------------+
// $Id: notice_tpl.inc.php,v 1.23.2.4 2012-01-05 17:40:47 touraine37 Exp $
...
function aff_expl($param) {
	...
	// champs retournés par $notice["notice_info"]->memo_exemplaires :
	//	expl_id, expl_cb,expl_cote,expl_statut,statut_libelle, expl_typdoc, tdoc_libelle, 
	//	expl_note, expl_comment, expl_section, section_libelle, expl_owner, lender_libelle, 
	//	codestat_libelle, expl_date_retour, expl_date_depot, expl_note, pret_flag, location_libelle
	$res="<table>";
	for ($i = 0 ; $i < count($notice["notice_info"]->memo_exemplaires) ; $i++) {
		$res.="<tr>";
				$res.="<td>".$notice["notice_info"]->memo_exemplaires[$i]->tdoc_libelle."</td>";
		$res.="<td>".$notice["notice_info"]->memo_exemplaires[$i]->location_libelle."</td>";
		$res.="<td>".$notice["notice_info"]->memo_exemplaires[$i]->section_libelle."</td>";
		$res.="<td>".$notice["notice_info"]->memo_exemplaires[$i]->expl_cote."</td>";
		$res.="<td>".$notice["notice_info"]->memo_exemplaires[$i]->expl_cb."</td>";
		$res.="<td>".$notice["notice_info"]->memo_exemplaires[$i]->statut_libelle."</td>";
		$res.="</tr>";
	}
	$res.="</table>";
	return $res;
...
function aff_parents_authors_by_type_with_tpl($param){
	...
	for($i=0 ; $i<count($notice['notice_info']->memo_notice_mere) ; $i++){
		$aut = array();
		$rqt_count="select count(*) as nb from responsability where responsability_notice='".$notice['notice_info']->memo_notice_mere[$i]."' ";
