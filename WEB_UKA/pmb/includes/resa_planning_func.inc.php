...
// +-------------------------------------------------+
// $Id: resa_planning_func.inc.php,v 1.15.4.3 2012-02-15 16:20:24 dgoron Exp $
...
function planning_list($idempr=0, $idnotice=0, $order="",$info_gestion=NO_INFO_GESTION) {
	...
	$q.= "concat(lower(empr_prenom), ' ', upper(empr_nom)) as empr_prenom_nom, id_empr, empr_cb, location_libelle,";
	...
	$q.= "FROM resa_planning LEFT JOIN notices ON resa_idnotice = notices.notice_id, empr, docs_location ";
	...
	while ($data = mysql_fetch_object($r)) {
		...
		switch ($info_gestion) {
			case GESTION_INFO_GESTION:
				...
				if (SESSrights & CIRCULATION_AUTH) $aff_final .= "<td><a href=\"./circ.php?categ=pret&form_cb=".rawurlencode($data->empr_cb)."\">".htmlentities($data->empr_prenom_nom,  ENT_QUOTES, $charset)."</a></td>"; 
				else $aff_final .= "<td>".htmlentities($data->empr_prenom_nom, ENT_QUOTES, $charset)."</td>";
		...
		switch ($info_gestion) {
			...
			case LECTEUR_INFO_GESTION:
				...
				if($data->resa_validee) {
					...
					$aff_final .= "<td class='sorttable_mmdd' style='text-align:center;'>";
					...
					$aff_final .= "<input type='button' class='bouton' onclick=\"openPopUp('./select.php?what=calendrier&caller=check_resa_planning&date_caller=".$resa_date_fin."&param1=form_resa_date_fin_".$data->id_resa."&param2=form_resa_date_fin_lib_".$data->id_resa."&auto_submit=NO&date_anterieure=YES&func_to_call=func_callback&id=".$data->id_resa."&sub_param1=2', 'resa_date_fin', 250, 300, -2, -2, 'toolbar=no, dependent=yes, resizable=yes')\" value='".$data->aff_resa_date_fin."' name='form_resa_date_fin_lib_".$data->id_resa."'>";
					$aff_final .= "</td>";
