...
// +-------------------------------------------------+
// $Id: export.class.php,v 1.20.2.1 2012-03-28 12:08:28 dgoron Exp $
	...
	function get_next_notice($lender = "", $td = array(), $sd = array(), $keep_expl = false, $params=array(), $force_diffusion=false) {
		...
		if ($this -> current_notice != -1) {
			...
			foreach ( $values as $field_id => $vals ) {
				...
				if($mes_pp->t_fields[$field_id]["EXPORT"] && $mes_pp->t_fields[$field_id]["OPAC_SHOW"]) { //champ exportable
					foreach ( $vals as $value ) {
						$subfields = array();
