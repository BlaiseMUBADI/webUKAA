...
// +-------------------------------------------------+
// $Id: cart_info.php,v 1.46.2.1 2012-01-04 16:45:12 dgoron Exp $
...
if (($id)&&(!$lvl)) {
	...
	switch ($lvl) {
		case "more_results":
			...
			switch ($mode) {
				case "tous" :
//					if(!$join){//Si join est vide soit on indexe pas les documents numériques ou on a fait une recherche *
//						if(!$rechercheMot){
//							$requete="select notice_id, ".stripslashes($pert)." from notices_global_index, notice_statut, notices ".stripslashes($clause)." and notice_id = num_notice group by notice_id ".stripslashes($tri);
//						}else{
//							$requete="select notice_id, ".stripslashes($pert)." from notices_mots_global_index, notice_statut, notices ".stripslashes($clause)." and notice_id = id_notice group by notice_id ".stripslashes($tri);
//						}
//						$searcher = new searcher_title(stripslashes($user_query));
//						$notices = $searcher->get_result();					
//					} else {
//						$requete="select distinct uni.notice_id from  ".stripslashes($join)." join notices n on uni.notice_id=n.notice_id  join notices_global_index on num_notice=uni.notice_id";
//					}
//					$message=add_query($requete);
				...
				case "title":	
				case "titre":
				//	$requete="select notice_id, ".stripslashes($pert)." from notice_statut, notices ".stripslashes($clause)." group by notice_id ".stripslashes($tri);
				//	$message=add_query($requete);
					$searcher = new searcher_title(stripslashes($user_query));
					$notices = $searcher->get_result();		
					add_notices_to_cart($notices);
