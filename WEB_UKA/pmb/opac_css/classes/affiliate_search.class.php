...
// +-------------------------------------------------+
// $Id: affiliate_search.class.php,v 1.4.2.1 2012-02-28 14:06:53 dbellamy Exp $
...
class affiliate_search {
	...
	function getNoticesResults() {
		...
		$this->results.=  "<hr />\n<center>".printnavbar($page, $nbepages, $url_page,$action,"catal_pag")."</center>";
		return $this->results;
    }
...
class affiliate_search_author extends affiliate_search {
	...
	function getNoticesAuthorityResults(){
		...
		$this->results ="
		<div id='aut_details'>
			<h3>".htmlentities($authority_title,ENT_QUOTES)."</h3>
			<div id='aut_see'>
				<div class='authorlevel2'>
					<h3>".htmlentities($aut_titre,ENT_QUOTES)."</h3>
				</div>
			</div>
			<div id='aut_details_liste'>
			<h3>".htmlentities($nb_results,ENT_QUOTES)." ".$msg['affiliate_search_external_authority_results'].implode($msg['affiliate_search_external_results_and'],$this->affiliate_source_name)."</h3>";
		...
		<div id='aut_details'>
			...
			<div id='aut_see'>
				<div class='collectionlevel2'>
					<h3>".$msg['collection_tpl_coll']." ".htmlentities($coll_name,ENT_QUOTES)."</h3>
					<ul>
						<li>".$msg['collection_tpl_publisher']." : ".htmlentities($publisher,ENT_QUOTES)."</li>";
		if($issn) $this->results .="
						<li>".$msg['collection_tpl_issn']." : ".htmlentities($issn,ENT_QUOTES)."</li>";
		$this->results .="				
					</ul>
				</div>
			</div>
		...
		<div id='aut_details'>
			...
			<div id='aut_see'>
				<div class='indexintlevel2'>
					<h3>".htmlentities($indexint_lib,ENT_QUOTES)."</h3>
				</div>
			</div>
			<div id='aut_details_liste'>
			<h3>".htmlentities($nb_results,ENT_QUOTES)." ".$msg['affiliate_search_external_authority_results'].implode($msg['affiliate_search_external_results_and'],$this->affiliate_source_name)."</h3>";
		...
		<div id='aut_details'>
			...
			<div id='aut_see'>
				<div class='categorylevel2'>
					<h3>".$msg['category']." ".htmlentities($categ_lib,ENT_QUOTES)."</h3>
				</div>
			</div>
			<div id='aut_details_liste'>
			<h3>".htmlentities($nb_results,ENT_QUOTES)." ".$msg['affiliate_search_external_authority_results'].implode($msg['affiliate_search_external_results_and'],$this->affiliate_source_name)."</h3>";
   ...
   function getAuthoritiesResults(){
 	...
 	function getNoticesAuthorityResults(){
		...
		if(count($joins)){
			...
			$nb_results=0;
			foreach($joins as $join){
				$res =mysql_query("select distinct notice_id, pert from ".$this->table_tempo." ".$join) or die(mysql_error());
				while($row = mysql_fetch_object($res)){
					$result[]=$row;
					$nb_results++;
				}
