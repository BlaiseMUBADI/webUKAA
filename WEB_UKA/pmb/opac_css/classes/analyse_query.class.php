...
// +-------------------------------------------------+
// $Id: analyse_query.class.php,v 1.40.2.12 2012-04-05 08:10:19 dgoron Exp $
			...
			if($tree[$i]->literal!=2){
				...
				switch($tree[$i]->operator){
					case "and" :
						...
						mysql_query($rqt)or die(print "<br>".$rqt."///".mysql_error());
						if($tree[$i]->not){
					...
					case "or" :
						...
						$last_table= "";
						mysql_query($rqt)or die(print "<br>".$rqt);
						break;
					default :
						$rqt = "create temporary table ".$temporary_table." ($field_id int, index using btree($field_id)) engine=memory $elem_query";
						mysql_query($rqt)or die(print "<br>".$rqt);
				...
				$query = "select $field_id from ".$temporary_table;
				if($last_table != ""){
...
//					}
					...
					if($tree[$i]->not == 1){
						...
						$res = mysql_query($temp_memory)or die(mysql_error());
	...
	function get_pert($notices_ids,$restrict=array(),$neg_restrict=false,$with_explnum=false,$return_query = false,$all_fields=false){
		...
		if(count($notices_ids)){
			...
			if($this->input !== "*"){
				...
				if(count($words)){
					$pert_query_words = "";
					$pert_query_words.= "select id_notice as notice_id, sum(!!pert!!) as pert from notices_mots_global_index where ";
					$where = "";
					...
					$queries[]= $pert_query_words.$where." group by id_notice ";
				}
				if(count($literals)){
					...
					$queries[]= $pert_query_literals.$where." group by id_notice ";
				}
