...
// +-------------------------------------------------+
// $Id: sort.class.php,v 1.30.2.2 2012-03-09 08:37:24 jpermanne Exp $
...
class dataSort {
	...
	function recupTriParId($id) {
		...
		if($id == "default"){
			$tab["nom_tri"] = "";
			$tab["tri_par"] = $opac_default_sort!= "" ? $opac_default_sort : "d_num_6,c_text_28";
			return $tab;
		}
			...
			case 'session':
				...
				for($i=0; $i<$nb_sort; $i++) {
					if (!in_array($i,$sort_ids)) {
						...
						$tab_sort[$j] = $_SESSION["sort".$this->sortName.$i];
						$j++;
					}
				...
				foreach($tab_sort as $k=>$v) {
					$_SESSION['sort'.$this->sortName.$k]=$v;
					if ($last_sort==$v) {
...
class sort {
	...
	function delete_useless($nomCol,$orderby,$need){
		...
		$res = mysql_query($query)or die(print $query);
		$keep = array();
		$nb_elem= 0;
...
//		mysql_query($add_index);
	...
	function genereRequeteUpdate($desTable, $nomTable, $nomChp, $nomColonneTempo) {
		...
		if (isset($desTable["GROUPBY"])) {
			$extractinfo_sql .= " GROUP BY ".$desTable["GROUPBY"][0]["value"]." order by null" ;		
		}		
