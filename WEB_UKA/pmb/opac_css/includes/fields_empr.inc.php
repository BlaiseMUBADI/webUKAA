...
// +-------------------------------------------------+
// $Id: fields_empr.inc.php,v 1.26 2011-10-10 08:31:51 dgoron Exp $
...
function aff_url_empr($field,&$check_scripts){
	...
	foreach ($values as $avalues) {
		...
		$ret.=" <input class=\"bouton\" type=\"button\" value=\"vérifier\" onclick='cp_chklnk($count,this);'>";
	...
	if ($options[REPEATABLE][0][value]) {
		...
		$ret.="<script>
			function add_custom_url_(field_id, field_name, field_size) {
				...
				chklnk.setAttribute('onclick','cp_chklnk(count,this);');
