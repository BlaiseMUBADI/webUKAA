...
// +-------------------------------------------------+
// $Id: ireps_pdl.inc.php,v 1.2.4.6 2012-02-10 09:53:36 dgoron Exp $
...
function search_other_function_clause() { //OK
	...
	$from="";
	if ($ireps_ss_type) {
		$where=" and (notices_custom_integer='".$ireps_ss_type."' and notices_custom_champ='3')";
	}
	
	if ($ireps_location) {
		$where.=" and notices_custom_origine in (select expl_notice from exemplaires where expl_location='".$ireps_location."' ";
	...
	if ($ireps_public) {
		$where.=" and (notices_custom_integer='".$ireps_public."' and notices_custom_champ='4')";
	}
	
	if ($ireps_expertise) {
		$where.=" and (notices_custom_small_text='".$ireps_expertise."' and notices_custom_champ='8')";
	}
	
	if ($ireps_indexint) {
		$from.=",notices";
		$where.=" and notices_custom_origine=notice_id";
		$where.=" and (indexint='".$ireps_indexint."')";
	}
	if ($ireps_ss_type || $ireps_location || $ireps_public || $ireps_expertise || $ireps_indexint) {
		$r.="select distinct notices_custom_origine as notice_id from notices_custom_values $from where 1 $where";
	}
	return $r;
}
