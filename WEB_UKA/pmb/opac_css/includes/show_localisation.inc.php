...
// +-------------------------------------------------+
// $Id: show_localisation.inc.php,v 1.54.2.3 2012-03-14 08:51:01 jpermanne Exp $
...
function affiche_notice_navigopac($requete){
	...
	print '<hr /><center>'.printnavbar($page, $nbepages, './index.php?lvl=section_see&id='.$id.'&location='.$location.(($back_surloc)?'&back_surloc='.urlencode($back_surloc):'').(($back_loc)?'&back_loc='.urlencode($back_loc):'').(($back_section_see)?'&back_section_see='.urlencode($back_section_see):'').'&page=!!page!!&nbr_lignes='.$nbr_lignes.'&dcote='.$dcote.'&lcote='.$lcote.'&nc='.$nc.'&main='.$main.'&ssub='.$ssub.'&plettreaut='.$plettreaut).'</center>';
}
...
if (!$location) {
	...
	print "<h3><span><a href=\"".$url_section_see."\"><img src='images/home.gif' border='0' align='bottom'/></a>$sur_location_link.$location_link</span></h3>";
