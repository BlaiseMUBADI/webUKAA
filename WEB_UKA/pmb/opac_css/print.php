...
// +--------------------------------------------------------------------------+
// $Id: print.php,v 1.47.2.1 2011-12-16 16:11:27 dgoron Exp $
		...
		for ($i=0; $i<count($cart_); $i++) {
			if($noti_tpl) {
				...
				if ($type=='PUBLIC') {
					...
					if ($vignette) $current->do_image($current->notice_public);
				} else {
					$current->do_isbd($short,$ex);
					if ($vignette) $current->do_image($current->notice_isbd);
				} 
